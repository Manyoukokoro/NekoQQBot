...
public class ChangeGachaCommand extends ManagerGroupCommand {
    ...
    public MessageChain execute(Member sender, Group subject, CommandAttr commandAttr, MessageChain messageChain) {
        if ("查询卡池".equals(commandAttr.getCommand())) {
            ...
            groupGacha.forEach(gacha -> singleMessages.append(gacha.getPollName()).append(":").append(String.valueOf(gacha.getUrP())).append(":").append(String.valueOf(gacha.getSsrP())).append(":").append(String.valueOf(gacha.getSrP())).append(":").append(String.valueOf(gacha.getRP())).append(":").append(String.valueOf(gacha.getNP())).append("\n"));
