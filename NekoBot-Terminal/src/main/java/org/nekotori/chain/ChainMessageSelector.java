...
public class ChainMessageSelector {
    ...
    public void registerChannel(Long groupId, ChannelHandler channelHandler) {
        ...
        final GroupCommandChannel build = GroupCommandChannel.builder().messageHisQueue(new ArrayDeque<>()).members(new ArrayList<>()).stages(channelHandler.getStages()).groupId(groupId).channelHandler(channelHandler).expireTime(System.currentTimeMillis() + 5 * 60 * 1000).build();
