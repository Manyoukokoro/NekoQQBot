...
public class /**/FiveChessHandler implements ChannelHandler {
    ...
    public static InputStream drawMap(Long groupId, int x, int y) {
        try {
            Resources resources = res.get(groupId);
            return ImageUtil.bufferedImageToInputStream(ImageUtil.drawFiveChess(resources.getField(), x, y));
    ...
    public void handleMessage(GroupCommandChannel channel, GroupMessageEvent groupMessageEvent) {
        ...
        if (channel.getNowStage().equals("B") && pair.get(groupMessageEvent.getSender().getId()).equals("B")) {
            ...
            MessageChain build = new MessageChainBuilder().append(Contact.uploadImage(groupMessageEvent.getSubject(), Objects.requireNonNull(drawMap(groupMessageEvent.getSubject().getId(), ints[1], ints[0])))).build();
        ...
        boolean isWin = FiveChessUtil.isWin(field);
        Long winnerId = 0L;
        if (isWin && "W".equals(channel.getNowStage())) {
            groupMessageEvent.getGroup().sendMessage("黑方胜利");
            clear(groupMessageEvent.getSubject().getId());
            chainMessageSelector.unregisterChannel(groupMessageEvent.getGroup().getId(), this.getClass().getAnnotation(HandlerId.class).value());
            winnerId = pair.entrySet().stream().filter(ss -> ss.getValue().equals("B")).map(Map.Entry::getKey).findAny().orElse(0L);
        }
        if (isWin && "B".equals(channel.getNowStage())) {
            groupMessageEvent.getGroup().sendMessage("白方胜利");
            clear(groupMessageEvent.getSubject().getId());
            chainMessageSelector.unregisterChannel(groupMessageEvent.getGroup().getId(), this.getClass().getAnnotation(HandlerId.class).value());
            winnerId = pair.entrySet().stream().filter(ss -> ss.getValue().equals("W")).map(Map.Entry::getKey).findAny().orElse(0L);
        }
        if (isFilled(field)) {
        ...
        if (winnerId != 0L) {
            ChatMemberDo chatMemberDo = chatMemberMapper.selectOne(Wrappers.<ChatMemberDo>lambdaQuery().eq(ChatMemberDo::getMemberId, winnerId).eq(ChatMemberDo::getGroupId, groupMessageEvent.getSubject().getId()));
